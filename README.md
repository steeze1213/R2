...
| 10 | [PACK — 이동 피킹 연동 자율주행 운반 로봇](#10-pack--이동-피킹-연동-자율주행-운반-로봇) | 팀 (4명) · 최종 | 2026.05.15 ~ 07.16 |
...
## 10. PACK — 이동 피킹 연동 자율주행 운반 로봇

### 프로젝트 주제

물건을 집는 리더 한 대와 나르는 팔로워 두 대가 한 팀으로 움직이는 다중 로봇 협업 운반 시스템. 리더(TurtleBot3 Waffle Pi + 자체 제작 6축 로봇팔)가 자율주행으로 이동해 비전 피킹으로 물건을 집어 팔로워에 싣고, 짐을 실은 팔로워(F1, F2)는 각자 벽면 ArUco 마커로 자기 위치를 계산하며 지정된 자리까지 스스로 이동한다. 과정 최종 프로젝트(STEP 8 프로젝트 실전)로 진행했다.

기존 고정형 피킹 설비와 달리 **이동하는 리더가 직접 물건을 집고 군집이 이어받아 운반**한다는 점, 그리고 고가 장비(로봇팔)는 1대만 두고 **저가 팔로워 N대가 운반을 분담**하는 역할 분담 협업 구조라는 점이 차별점이다. 앞선 공기청정 로봇(PURE) 프로젝트의 A* + Pure Pursuit 주행 자산을 재활용해 확장했다.

### 팀 구성 및 본인 역할

4명 (팀 MYMY) / **로봇팔 서브시스템 전반 + 비전 피킹 + 측위 연동 담당**
- 자체 제작 6축 로봇팔 펌웨어 구현 (Arduino + PCA9685, 각도-펄스 변환, 채널별 안전범위 클램프, 순기구학(FK) 충돌가드, 비상정지)
- MQTT 무선 제어 게이트웨이 구현 (`pack/arm/cmd` 토픽 → USB 시리얼 중계)
- 비전 피킹 구현: eye-in-hand 카메라로 박스의 색·위치 인식 → 픽셀 좌표를 로봇팔 기준 3D 좌표로 변환 → **역기구학(IK)** 으로 관절 각도 계산 → 파지·적재
- 3박스 순차 피킹 및 미션 매니저 연동 (작업 완료 시 `load_done` 회신)
- 측위 연동 보조
...
| 계층 | 구성요소 | 역할 | 담당 |
|------|---------|------|------|
| 리더 | TurtleBot3 Waffle Pi | Nav2/AMCL·A* 자율주행 + 미션 지휘(상태머신) | 윤우영 |
| 로봇팔 | 자체 제작 6축 팔 (Arduino+PCA9685) + Pi Camera | 색·위치 인식 → IK 파지 → 팔로워 적재 | 길민준 |
| F1 | Arduino 4WD + 쿼드러처 엔코더 | 리더 후면 마커(159) 추종 → 지정 위치 단독 이동 | 김아영 |
| F2 | Arduino 4WD + 쿼드러처 엔코더 | F1 마커(158) 추종 → 재타겟 → A* 접근·이동 | 김아영·안효민 |
| 측위 | ROS2 + OpenCV solvePnP | 멀티마커 PnP + 엔코더 오도메트리 융합 위치추정 | 김아영 |
| 통신·웹 | Mosquitto MQTT + Flask/rosbridge | 미션 명령·로봇팔 제어·상태 실시간 관제 | 안효민 |

### 기능적 요소

**로봇팔 비전 피킹 (본인 파트)**
- eye-in-hand 카메라 HSV 색검출로 좌/중/우 3박스의 색과 위치 인식
- 픽셀 좌표 → 카메라 보정·거리 기반 로봇팔 기준 3D 좌표 변환
- 역기구학(IK)으로 6축 관절 각도 계산 후 정밀 파지, 팔로워 후면에 적재
- 순기구학(FK) 충돌가드로 집게끝이 베이스 반경 진입 시 자동 정지, 비상정지 내장
- MQTT(`pack/arm/cmd`)로 무선 제어, 완료 시 미션 매니저에 `load_done` 회신

**편대 추종 · 자율주행**
- 벽면 ArUco 마커를 여러 개 동시에 푸는 멀티마커 solvePnP + 엔코더 융합으로 각 로봇 개별 측위
- 앞차 후면 마커 직접 추종을 우선하고, 마커를 놓치면 맵 기반 위치로 복구 (하이브리드 추종)
- F2는 정적 맵 기반 A* + Pure Pursuit로 접근·지정 위치 이동 경로를 스스로 계획
- 미션 상태머신으로 편대 구성 → 이동·피킹 → 적재 → 이탈 → 지정 위치 이동 전 과정 자동 전환
- 각 로봇은 mode manager / command mux로 '한 번에 하나의 명령'만 수신 (안전·전압 강하 방지)
- 웹 대시보드로 3대 위치·상태 실시간 모니터링 및 미션 시작/정지 제어

### 활용 기술 및 도구

| 구분 | 내용 |
|------|------|
| **로봇** | TurtleBot3 Waffle Pi, 자체 제작 6축 로봇팔 |
| **MCU/구동** | Arduino Uno ×3, PCA9685(I2C PWM), 서보 6채널, 4WD + 쿼드러처 엔코더 ×2 |
| **OS/ROS2** | Ubuntu 22.04, ROS2 Humble, Nav2, AMCL |
| **비전** | OpenCV (ArUco, HSV 색검출), Pi Camera (eye-in-hand) |
| **측위/경로** | 멀티마커 solvePnP, 엔코더 오도메트리 융합, A* + Pure Pursuit (Safety Cost) |
| **로봇팔** | 역기구학(IK), 순기구학(FK) 충돌가드, 각도-펄스 매핑 |
| **통신** | Mosquitto MQTT, rosbridge WebSocket, USB Serial |
| **웹** | Flask + rosbridge (실시간 관제 대시보드) |

### 프로젝트 주요 성과

- 이동 피킹 + 군집 추종 운반을 결합한 다중 로봇 협업 시스템 완전 구현
- 자체 제작 6축 로봇팔을 역기구학 기반으로 제어, eye-in-hand 비전으로 색·위치 인식 후 정밀 파지·적재
- MQTT 하나로 미션 명령·로봇팔 제어·상태 모니터링을 통합 (서버 로직 없이 모듈끼리 직접 통신 → 서버가 끊겨도 자율 동작)
- 멀티마커 solvePnP + 엔코더 융합으로 각 로봇이 개별 자율 측위 (원격 조종 아님)
- F2 정적 맵 A*로 접근·복귀 경로 자율 계획 (개별 자율주행)
- 미션 상태머신으로 편대 구성부터 지정 위치 복귀까지 전 과정 자동 전환

### 힘들었던 점 및 아쉬웠던 점

- 단계 전환이 대시보드 수동 트리거에 의존 — 완전 자동화까지는 미완
- 추종이 마커에 의존해 조명·가림·마커 없는 구간에 취약
- 편대 정렬·간격 튜닝에 많은 시간 소요
- eye-in-hand 카메라가 수평을 봐 대기자세에서 물건이 화면에 안 잡히는 구조적 한계 발견 (자세 재설계 필요)

### 기술적 개선 가능성

- 추종 방식 파라미터화 — 상황별 마커 / SLAM / 라인 추종 선택형 전환
- 팔로워 N대 확장 및 1대 고장 시 순번 자동 대체
- 인계·적재/하역 완전 자동화 (수동 트리거 제거)
- 라이다 기반 동적 장애물 실시간 감지 및 A* 재계획
- 리더의 '집는 작업' 모듈만 교체해 물류 / 농업 / 병원 등 범용 적용
